                  ...
                  int nrhs, const mxArray *prhs[])
{
  ...
  subspace_iter_sv_both_alternate_(a, u, v, s, &m, &n, &l, &p,
                &percentage, &maxit, &eps, 
                res_sv, it_sv, &it, &n_sv, &ierr );
 
  if(n_sv==0)
  { 
     plhs[0] = mxCreateDoubleMatrix( (mwSize)n_sv, (mwSize)n_sv, mxREAL);
     plhs[1] = mxCreateDoubleMatrix( (mwSize)n_sv, (mwSize)n_sv, mxREAL);
     plhs[2] = mxCreateDoubleMatrix( (mwSize)n_sv, (mwSize)n_sv, mxREAL);
  }   
  else 
  {  				
